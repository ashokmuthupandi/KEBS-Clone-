...
const NavbarCalender = (props) => {
  ...
  const handleMonthClick = (event) => {
    if (event.target.classList.contains("Navbar-calender-month-box-list-item")) {
  ...
  const handleInputChange = (week, index, value) => {
    ...
    // Update the inputBackgroundColors based on the entered value for the specific day
    setInputBackgroundColors((prevColors) => ({
      ...prevColors,
      [week]: [
        ...prevColors[week].slice(0, index),
        value === "8" ? "#20BF6B" : value.toLowerCase() === "wo" ? "#D1D8E0" : "",
        ...prevColors[week].slice(index + 1),
      ],
    }));
  };
                    ...
                    {inputValues[selectedWeek].map((value, index) => (
                      <li key={index}>
                        <input
                          ...
                          style={{ backgroundColor: inputBackgroundColors[selectedWeek][index] }}
                        />
                      </li>
                    ))}
                  </ul>
